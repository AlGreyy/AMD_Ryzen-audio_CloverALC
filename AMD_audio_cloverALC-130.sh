#!/bin/sh
# Adapted by AlGrey for AMD AM4 motherboards.
# Credit to Ydeng for the AMD patch.
# ---Original header---
...
# verify and remove VoodooHDA.kext
ioreg > /tmp/vooodo.txt

if [[ $(cat /tmp/vooodo.txt | grep -c "VoodooHDADevice") != 0 ]]; then
echo ""
echo "Error: VoodooHDA detected! Removing VoodooHDA.kext ..."

if [ -d "$gCloverDirectory/$gSysFolder/VoodooHDA.kext" ]; then
sudo rm -Rf "$gCloverDirectory/$gSysFolder/VoodooHDA.kext"
echo "$gCloverDirectory/$gSysFolder/VoodooHDA.kext deleted"
fi

if [ -d "$gCloverDirectory/kexts/Other/VoodooHDA.kext" ]; then
sudo rm -Rf "$gCloverDirectory/kexts/Other/VoodooHDA.kext"
echo "$gCloverDirectory/Other/VoodooHDA.kext deleted"
fi

if [ -d "$gLibraryDirectory/VoodooHDA.kext" ]; then
sudo rm -Rf "$gLibraryDirectory/VoodooHDA.kext"
cho "$gLibraryDirectory/VoodooHDA.kext deleted"
fi

if [ -d "$gLibraryDirectory/VoodooHDA.kext" ]; then
sudo rm -Rf "$gLibraryDirectory/VoodooHDA.kext"
echo "$gLibraryDirectory/VoodooHDA.kext deleted"
...
if [ -d "$gExtensionsDirectory/VoodooHDA.kext" ]; then
sudo rm -R "$gExtensionsDirectory/VoodooHDA.kext"
echo "$gExtensionsDirectory/VoodooHDA.kext deleted"
...
if [[ $(ioreg -rxn HDEF | grep IOName | sed -e 's/.*pci//' -e 's/\"//' | grep -c "1002") = 0 ]] && [[ $(ioreg -rxn HDEF | grep IOName | sed -e 's/.*pci//' -e 's/\"//' | grep -c "1022") = 0 ]]; then
echo "Error: Something is wrong with your HDEF patch, please report issue. Save output (CMD + A) to a text file. Also, dump IOReg and attach config.plist."
sudo rm -R /tmp/HDEF.txt
exit 1
fi
echo "Your audio device (VendorID,ProductID): $(ioreg -rxn HDEF | grep IOName | sed -e 's/.*pci//' -e 's/\"//') "
...
if [[ $(cat /tmp/HDEF.txt | grep -c "HDAS") != 0 ]]; then
echo "Error: macOS does not support dsdt/HDAS; install Clover patch or ssdt and reboot"
audiodevice=HDAS
fi

ioreg -rw 0 -p IODeviceTree -n CAVS > /tmp/HDEF.txt
if [[ $(cat /tmp/HDEF.txt | grep -c "CAVS") != 0 ]]; then
echo "Error: macOS does not support dsdt/CAVS; install Clover patch or ssdt and reboot"
audiodevice=CAVS
fi

ioreg -rw 0 -p IODeviceTree -n ALZA > /tmp/HDEF.txt
if [[ $(cat /tmp/HDEF.txt | grep -c "ALZA") != 0 ]]; then
echo "Error: macOS does not support dsdt/ALZA; install Clover patch or ssdt and reboot"
audiodevice=ALZA
fi
...
if [[ $(cat /tmp/HDEF.txt | grep -c "D01A") != 0 ]]; then
echo "Error: macOS does not support dsdt/D01A;"
echo "Your audio device (VendorID,ProductID): $(ioreg -rxn D01A | grep IOName | sed -e 's/.*pci//' -e 's/"//') "
audiodevice=D01A
amdpatch=1
fi
...
echo "Adding $audiodevice to HDEF patch to config.plist"
sudo /usr/libexec/PlistBuddy -c "Print ':ACPI:DSDT:Patches:$amdpatch'" /tmp/config_amd_hda.plist -x > "/tmp/hdef.plist"

atpexisting=$(sudo /usr/libexec/PlistBuddy -c "Print ':ACPI:DSDT:Patches'" /tmp/config.plist)
if [ -z "${atpexisting}" ]; then
sudo /usr/libexec/PlistBuddy -c "Add :ACPI:DSDT:Patches dict" /tmp/config.plist
fi

acpitotal=$(sudo /usr/libexec/PlistBuddy -c "Print ':ACPI:DSDT:Patches:'" /tmp/config.plist | grep -c "Find")
sudo /usr/libexec/PlistBuddy -c "Add :ACPI:DSDT:Patches:$acpitotal dict" /tmp/config.plist
sudo /usr/libexec/PlistBuddy -c "Merge /tmp/hdef.plist ':ACPI:DSDT:Patches:$acpitotal'" /tmp/config.plist
...
sudo rm -R /tmp/HDEF.txt
sudo rm -R /tmp/hdef.plist
exit 1
fi



# verify and remove AppleHDADisabler.kext
ioreg -rn -p -d  IODeviceTree -n HDEF -d 2  > /tmp/disabler.txt

if [[ $(cat /tmp/disabler.txt | grep -c "Disabler") != 0 ]]; then
echo ""
echo "Error: AppleHDADisabler detected! Removing AppleHDADisabler.kext ..."

if [ -d "$gCloverDirectory/$gSysFolder/AppleHDADisabler.kext" ]; then
sudo rm -Rf "$gCloverDirectory/$gSysFolder/AppleHDADisabler.kext"
echo "$gCloverDirectory/$gSysFolder/AppleHDADisabler.kext deleted"
fi

if [ -d "$gCloverDirectory/kexts/Other/AppleHDADisabler.kext" ]; then
sudo rm -Rf "$gCloverDirectory/kexts/Other/AppleHDADisabler.kext"
echo "$gCloverDirectory/Other/AppleHDADisabler.kext deleted"
fi

if [ -d "$gLibraryDirectory/AppleHDADisabler.kext" ]; then
sudo rm -Rf "$gLibraryDirectory/AppleHDADisabler.kext"
echo "$gLibraryDirectory/AppleHDADisabler.kext deleted"
...
if [ -d "$gExtensionsDirectory/AppleHDADisabler.kext" ]; then
sudo rm -R "$gExtensionsDirectory/AppleHDADisabler.kext"
echo "$gExtensionsDirectory/AppleHDADisabler.kext deleted"
